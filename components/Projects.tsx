...
export function Projects() {
  return (
    <Section id="projetos" className="overflow-hidden bg-muted/40">
      <Blob shape={2} className="-left-40 top-24 h-96 w-96 bg-lilac/35" />

      <SectionHeading
        title="Projetos de investigação"
        intro="Projetos em que trabalhei durante o mestrado e a licenciatura, sempre com a recolha e a análise de dados no centro."
      />
          ...
          >
            <span
              className={`inline-block rounded-full px-3 py-1 text-sm font-bold ${
                p.context === "Mestrado" ? "bg-primary/10 text-primary" : "bg-secondary/10 text-clay-ink"
              }`}
            >
              {p.context}
            </span>
